...
--Set the recovery model to full for dms_sample_baseball - required for replication
ALTER DATABASE [dms_sample_baseball] SET RECOVERY FULL WITH NO_WAIT
GO

--Set the recovery model to full for dms_sample_football - required for replication
ALTER DATABASE [dms_sample_football] SET RECOVERY FULL WITH NO_WAIT
GO
...
exec sp_adddistpublisher @publisher = @@SERVERNAME, @distribution_db = N'distribution', @security_mode = 1, @working_directory = N'C:\Program Files\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQL\ReplData', @trusted = N'false', @thirdparty_flag = 0, @publisher_type = N'MSSQLSERVER'
GO
